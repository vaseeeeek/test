...
class shopProductsetsPluginFrontendGetProductSkusAction extends waViewAction
{
    private $ignore_stock_count = null;
    ...
    public function execute()
    {
        ...
        $stocks = (method_exists('shopHelper', 'getStocks') ? shopHelper::getStocks(true) : shopProductsetsPluginMigrate::getStocks());

        if ($product) {
            $product = new shopProduct($product, true);
            $this->prepareProduct($product);
            $this->view->assign('currency_info', (new shopProductsetsPluginHelper())->getCurrencyInfo());
            $this->view->assign('stocks', $stocks);
            $this->view->assign('ruble_sign', waRequest::post('ruble_sign', 0));
            $this->view->assign('type', waRequest::post('type', 'select'));
            $this->view->assign('image_size', $this->getImageSize());
            $this->view->assign('version', wa('shop')->getPlugin('productsets')->getVersion());
            $this->view->assign('plugin_url', wa()->getAppStaticUrl('shop') . "plugins/productsets");
        }

        $templates = (new shopProductsetsPluginHelper())->getTemplates();
        $this->setTemplate($templates['product_skus']['frontend_path']);
    }

    private function prepareProduct(shopProduct $product)
    {
        $skus = $product->skus;
        $has_virt_stocks = method_exists('shopHelper', 'fillVirtulStock');
        $this->ignore_stock_count = $this->getConfig()->getGeneralSettings('ignore_stock_count');
        ...
        foreach ($skus as $s_id => $s) {
            ...
            if (!$s['available'] || ($product->sku_type == shopProductModel::SKU_TYPE_FLAT && !$this->isProductAvailable($s['count'], ifempty($settings, array())))) {
                unset($skus[$s_id]);
            }
        }
        ...
        if (!empty($public_stocks)) {
            ...
            if ($count === 0 && !$this->ignore_stock_count) {
                $product->status = 0;
            }
        }
            ...
            foreach ($sku_features as $sku_id => $sf) {
                ...
                $sku_selectable[$sku_f] = array(
                    ...
                    'available' => $product->status && $sku['available'] &&
                        ($this->ignore_stock_count || $sku['count'] === null || $sku['count'] > 0),
                    'image_id' => (int) $sku['image_id'],
                );
                $sku_selectable[$sku_f]['count'] = $sku_selectable[$sku_f]['available'] && !$this->ignore_stock_count && $sku['count'] !== null ? $sku['count'] : '';
                if ($sku_selectable[$sku_f]['available']) {
                    $sku_selectable[$sku_f]['available'] = $this->isProductAvailable($sku_selectable[$sku_f]['count'], ifempty($settings, 'quantity', 0));
                }
     ...
     * @return int|null
     */
    private function countOfSelectedStocks($public_stocks, $skus)
    {
        $count = null;
        foreach ($skus as $sku) {
    ...
    private function isProductAvailable($product_quantity, $settings = array())
    {
        if (!$this->ignore_stock_count && $product_quantity !== null && $product_quantity !== '' && $product_quantity > 0
            && empty($settings['choose_quantity']) && !empty($settings['quantity']) && $settings['quantity'] > $product_quantity) {
            return false;
        }
        return true;
    }
    ...
    {
        $settings = (new shopProductsetsPluginHelper())->getSettings();
        $image_width = !empty($settings['frontend_image_size']['width']) ? (int) $settings['frontend_image_size']['width'] : 0;
        $image_height = !empty($settings['frontend_image_size']['height']) ? (int) $settings['frontend_image_size']['height'] : (isset($settings['frontend_image_size']['height']) && $settings['frontend_image_size']['height'] == '' ? '' : 0);
        $image_size = $image_width . ($image_height !== '' ? 'x' . $image_height : '');
        if ($image_size == '0x0') {
            $image_size = shopProductsetsProductData::FRONTEND_IMAGE_SIZE;
        }
        return $image_size;
    }
