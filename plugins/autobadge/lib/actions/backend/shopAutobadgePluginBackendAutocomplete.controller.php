...
class shopAutobadgePluginBackendAutocompleteController extends waController
{
    ...
    public function productAutocomplete($q)
    {
        ...
        if ($count < $this->limit) {
            ...
            if ($skus) {
                foreach ($skus as $s) {
                    $products[] = array(
                        'label' => shopAutobadgeHelper::secureString($s['name']) . ' <span class="hint">' . ($s['sku_name'] ? shopAutobadgeHelper::secureString($s['sku_name']) . ' ' : '') . ($s['sku'] ? ' (' . shopAutobadgeHelper::secureString($s['sku']) . ')' : '') . (!$s['sku_name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : '') . '</span>',
                        'name' => shopAutobadgeHelper::secureString($s['name']),
                        'sku_name' => ($s['sku_name'] ? shopAutobadgeHelper::secureString($s['sku_name']) : ($s['sku'] ? shopAutobadgeHelper::secureString($s['sku']) : (!$s['sku_name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : ''))),
                        'sku_id' => $s['id'],
                        'id' => $s['product_id']
                    );
                }
        ...
        foreach ($products as &$p) {
            $p['name'] = $p['label'] = shopAutobadgeHelper::secureString($p['name']);
        }
        ...
        if (waRequest::get('with_skus')) {
            ...
            foreach ($product_skus as $s) {
                $product_and_skus[] = array(
                    'label' => $products[$s['product_id']]['name'] . ' <span class="hint">' . ($s['name'] ? shopAutobadgeHelper::secureString($s['name']) . ' ' : '') . ($s['sku'] ? ' (' . shopAutobadgeHelper::secureString($s['sku']) . ')' : '') . (!$s['name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : '') . '</span>',
                    'name' => $products[$s['product_id']]['name'],
                    'sku_name' => ($s['name'] ? shopAutobadgeHelper::secureString($s['name']) : ($s['sku'] ? shopAutobadgeHelper::secureString($s['sku']) : (!$s['name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : ''))),
                    'sku_id' => $s['id'],
                    'id' => $s['product_id']
                );
            }
