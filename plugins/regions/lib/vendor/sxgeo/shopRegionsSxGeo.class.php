...
class shopRegionsSxGeo {
	...
	public function __construct($db_file = 'SxGeo.dat', $type = self::SXGEO_FILE){
		...
		if(substr($header, 0, 3) != 'SxG') throw new Exception("Can't open {$db_file}\n");
		...
		if($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) throw new Exception("Wrong file format {$db_file}\n");
