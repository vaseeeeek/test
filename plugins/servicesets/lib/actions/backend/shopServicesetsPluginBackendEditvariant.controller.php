<?php
class shopServicesetsPluginBackendEditvariantController extends waController
{
    public function execute()
    {
        $model = new shopServicesetsPluginVariantsModel();
        ...
        if ($data['reset-image-one'] == 'reset') {
            ...
            if ($image->uploaded() && in_array(strtolower($image->extension), array('jpg', 'jpeg', 'png', 'gif'))) {
                $fileName = $data['id_variants'] . '-1.' . strtolower($image->extension);
                $imagePath = wa('shop')->getAppPath('plugins/servicesets/', 'shop') . 'img/variant/' . $fileName;
                try {
                    $image->waImage()
                        ->save($imagePath);
            ...
            if ($image->uploaded() && in_array(strtolower($image->extension), array('jpg', 'jpeg', 'png', 'gif'))) {
                $fileName = $data['id_variants'] . '-2.' . strtolower($image->extension);
                $imagePath = wa('shop')->getAppPath('plugins/servicesets/', 'shop') . 'img/variant/' . $fileName;
                try {
                    $image->waImage()
                        ->save($imagePath);
        ...
        if ((bool) $model->countByField('id_variants', $id)) {
            $model->updateByField('id_variants', $id, $data);
        } else {
            $model->insert($data, 2);
        }
