...
require_once __DIR__ . '/../../../../../../wa-apps/shop/plugins/cwebp/lib/classes/shopCwebpPluginConvert.class.php';

$requested = urldecode($_SERVER['DOCUMENT_ROOT'] . $_SERVER['REQUEST_URI']);
if (($i = strpos($requested, '?')) !== false) {
    $requested = substr($requested, 0, $i);
}
preg_match(';(\d)(\d)/webp/;m', $requested, $m);
$ext = ['jpg', 'JPG', 'jpeg', 'JPEG', 'png', 'PNG'][$m[2]];

if ($m[1] == 0) {
    $source = substr(str_replace("products/$m[1]$m[2]/webp", 'products', $requested), 0, -4) . $ext;
} else {
    $source = substr(str_replace("wa-data/public/shop/products/$m[1]$m[2]/webp/", 'wa-', $requested), 0, -4) . $ext;
}

if (file_exists($source)) {
    (new shopCwebpPluginConvert($source, $requested, 'Fast thumb'))->convertAndRead();
}

$_SERVER['REQUEST_URI'] = substr(str_replace("products/$m[1]$m[2]/webp", 'products', $_SERVER['REQUEST_URI']), 0, -4) . $ext;

// Original thumb not found so we need to load framework to make product thumb. Original shop/lib/config/data/thumb.php goes next:
...
// Now we have thumb in $thumb_path:
if ($file && file_exists($thumb_path)) {
    (new shopCwebpPluginConvert($thumb_path, $requested, 'On demand product image'))->convertAndRead();
} else {
