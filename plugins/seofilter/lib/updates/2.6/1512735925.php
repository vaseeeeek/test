...
try
{
	$model->exec('SELECT cache_generator_id FROM shop_seofilter_sitemap_cache_queue LIMIT 1');
}
catch (Exception $e)
{
	$sql = '
ALTER TABLE `shop_seofilter_sitemap_cache_queue`
	ADD COLUMN `cache_generator_id` VARCHAR(16) NULL DEFAULT NULL AFTER `lock_timestamp`,
	ADD INDEX `cache_generator_id` (`cache_generator_id`);
';
