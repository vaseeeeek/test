	...
	public static function initVariables($control, $variables)
	{
		...
		foreach($variables as $key => $description) {
			if($key != '_locale') {
				...
				$control .= '<td class="key" valign="top"><a href="#">{$' . $key . '}</a></td><td class="description"><span class="hint">' . (is_string($description) ? $description : $description['description']) . '</span></td>';
				$control .= '</tr>';
...
HTML;
	...
	public static function initReturnDefaultLink($control, $control_name, $settings, $editor = false, $variables = false)
	{
		...
		$string = is_string($settings) ? $settings : 'Вернуть значение по умолчанию';
