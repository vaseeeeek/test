...
class shopQuickorderPluginSypexgeo {
	protected $fh;
	protected $ip1c;
	protected $info;
	protected $range;
	...
	public function __construct($db_file = 'SxGeoCity.dat', $type = SXGEO_FILE){
	    $path_to_file = wa()->getAppPath('plugins/quickorder/lib/classes/libraries/sypexgeo/', 'shop');
		$this->fh = fopen($path_to_file . $db_file, 'rb');
		...
		if(substr($header, 0, 3) != 'SxG') {
		    throw new waException("Can't open {$db_file}\n");
		}
		...
		if($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) {
		    throw new waException("Wrong file format {$db_file}\n");
        }
