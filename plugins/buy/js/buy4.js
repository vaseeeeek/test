...
$.buy = $.buy || function(buy_params) {
    ...
    if(buy_params.shipping_id) {
        ...
        $('.checkout-shipping-process').click(function () {
            var action,
                $address = $('#customer_address'),
                hasErrors = false;
            if($(this).data('calc')) {

                $address.find('.form-group').find('.error').remove();

                $address.find('input,select').each(function (i, v) {
                    var $block, $error;

                    if($(v).prop('required') && !$(v).val()) {
                        $block = $(v).closest('.form-group');
                        $error = $block.find('.error');

                        if(!$error.length) {
                            $error = $('<div class="error"/>');
                            $error.text('Необходимо заполнить');
                            $block.append($error);
                        }
                        hasErrors = true;
                    }
                ...
                $('[data-save-address]:first').trigger('change');
                updateShiping();
                $(this).data('calc', 0).val('Изменить адрес').addClass('checkout-shipping-change');
                $address.find('input,select').each(function (i, v) {
                    var $this = $(v),
                ...
                $(this).data('calc', 1).val('Рассчитать доставку').removeClass('checkout-shipping-change');
                $address.find('input,select').each(function (i, v) {
                    var $this = $(v),
                        $input;
                    ...
                    $input = $address.find('[type="hidden"][name="'+$this.data('name')+'"]');
                    $input.remove();
                ...
                var $r = $('.shipping-method').find('.rate').show();
                $r.find('.price').html('Загрузка... <i class="icon16 loading"></i>').show();
                $r.find('.hint').hide();
                action = 'slideUp';
            }
            $('.shipping-method, .checkout-step-payment, .checkout-step-confirmation')[action]();
        });
