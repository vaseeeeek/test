...
/**
 * @link http://php.net/manual/ru/function.idn-to-utf8.php#112097
 */
class shopListfeaturesPluginIdna
{
    ...
    private static function utf8($cp)
    {
        ...
        if ($cp < 65536) {
            return chr(224 + ($cp >> 12))
                .chr(128 + (($cp >> 6) & 63))
                .chr(128 + ($cp & 63));
        }
        if ($cp < 2097152) {
            return chr(240 + ($cp >> 18))
                .chr(128 + (($cp >> 12) & 63))
                .chr(128 + (($cp >> 6) & 63))
                .chr(128 + ($cp & 63));
        }
    ...
    public static function decode($name)
    {
        static $callback;
        if (!$callback) {
            if (method_exists('waIdna', 'decode')) {
                $callback = array(new waIdna(), 'decode');
            } elseif (function_exists('idn_to_utf8')) {
                $callback = 'idn_to_utf8';
            } else {
                $callback = 'self::decodePart';
            }
        }
        return implode('.', array_map($callback, explode('.', $name)));
    }
}
